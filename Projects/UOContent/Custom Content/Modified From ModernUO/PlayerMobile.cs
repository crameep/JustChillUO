...
namespace Server.Mobiles
{
    ...
    {
        // ### Skill Template System Edits ###
        private List<SkillTemplate> _skillTemplates;
        ...
        {
            get => _maxTemplates;
            set => _maxTemplates = value;
        }
        // ### END Skill Template System Edits ###
        ...
        */
        ...
        public override void Deserialize(IGenericReader reader)
        {
            ...
            // ### Skill Template System Modification
            // Deserialize max templates and skill templates
            if (version >= 1)
            {
                _maxTemplates = reader.ReadInt();
                int templateCount = reader.ReadInt();
                _skillTemplates = new List<SkillTemplate>(templateCount);
                for (int i = 0; i < templateCount; i++)
                {
                    ...
                    for (int j = 0; j < skillCount; j++)
                    {
                        ...
                        var skillValue = reader.ReadDouble();
                        template.Skills[skillName] = skillValue;
                    }
                    template.Strength = reader.ReadInt();
                    template.Dexterity = reader.ReadInt();
                    template.Intelligence = reader.ReadInt();
                    _skillTemplates.Add(template);
                }
            }
            else
            {
        ...
        // ### End Skill Template System Modification ###
        ...
        public override void Serialize(IGenericWriter writer)
        {
            ...
            // ### Skill Template System Modification ###
            // Serialize max templates and skill templates
            writer.Write(_maxTemplates);
            writer.Write(_skillTemplates.Count);
            foreach (var template in _skillTemplates)
            {
                writer.Write(template.Name);
                writer.Write(template.Skills.Count);
                foreach (var kvp in template.Skills)
                {
                    writer.Write((int)kvp.Key);
                    writer.Write(kvp.Value);
                }
                writer.Write(template.Strength);
                writer.Write(template.Dexterity);
                writer.Write(template.Intelligence);
            }
            writer.Write(_maxTemplates);
            writer.Write(_skillTemplates.Count);
            foreach (var template in _skillTemplates)
