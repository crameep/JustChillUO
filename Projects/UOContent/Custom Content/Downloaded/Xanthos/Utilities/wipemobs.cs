            ...
            for ( int i = 0, index = page * EntryCount; i < EntryCount && index < m_Names.Count; ++i, ++index )
            {
                ...
                if (item != null)
                {
                    ...
                    if (!item.Deleted)
                    {
                        ...
                        AddImageTiled(x, y, ButtonWidth, EntryHeight, ButtonGumpID);
                        AddButton(x + GoOffsetX, y + GoOffsetY, GoButtonID1, GoButtonID2, i + 1004);
                    }
                }
            }
                        ...
                        if ( index >= 0 && index < m_Names.Count )
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if (s != null)
                                    {
                                        from.SendMessage($"Going to {s.Name}." );
        ...
        private class WipeAllGump : Gump
        {
            ...
            public override void OnResponse( Server.Network.NetState sender, in RelayInfo info )
            {
                if ( info.ButtonID == 2 )
                {
                    ...
                    m_From.SendMessage($"Deleted {m_Names.Count} Mobiles.");
                }
            }
        }
    }
