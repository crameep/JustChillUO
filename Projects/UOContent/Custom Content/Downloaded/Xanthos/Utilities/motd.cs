...
namespace Xanthos.Utilities
{
    public class NewsGump : Gump
    {
        ...
        public static void Initialize()
        {
            ...
            CommandHandlers.Register("MOTD", AccessLevel.Administrator, MOTD_OnCommand);
        }
        ...
        private static void SetAllTags(Mobile from, bool toTrue)
        {
            ...
            foreach (Mobile mobile in World.Mobiles.Values)
            {
                ...
                if ((account = (Account)from.Account) == null)
                    continue;

                account.SetTag("motd", boolValue);
            }
            from.SendMessage(kLabelColor, "All players" + (toTrue ? " will " : " will not ") + "see the MOTD Message at next login.");
        }
        ...
        private NewsGump(Mobile mobile, string body, string[] archiveList, int archiveCount, bool admin) : base(100, 100)
        {
            ...
            m_ArchiveCount = (archiveList == null ? 0 : archiveCount);
            m_ArchivesLoaded = 0;
            m_TotalPages = 0;
        }
        ...
        internal class AccountGump : Gump
        {
            public AccountGump(Mobile from, NetState state, bool showOnlyCredits) : base(30, 20)
            {
                ...
                if (!showOnlyCredits)
                {
                    ...
                    AddLabel(205, 180, NewsGump.kDataColor, state.Flags.ToString());
                    AddLabel(205, 200, NewsGump.kLabelColor, "Client Patch");
                    AddLabel(205, 215, NewsGump.kDataColor, state.Version == null ? "(null)" : state.Version.ToString());
                ...
                AddLabel(355, 75, NewsGump.kDataColor, "The RunUO Community");
