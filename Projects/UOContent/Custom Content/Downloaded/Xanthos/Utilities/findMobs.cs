...
namespace Xanthos.Utilities
{
    public class FindMobsGump : Gump
    {
                ...
                for ( int i = 0, index = page * kEntryCount; i < kEntryCount && index < m_Names.Count; ++i, ++index )
                {
                    ...
                    if (item != null)
                    {
                        AddLabelCropped(
                            x + kTextOffsetX,
                            y,
                            kEntryWidth - kTextOffsetX,
                            kEntryHeight,
                            0x58,
                            item != null && item.Deleted
                                ? "(deleted)"
                                : (string.Format(
                                    "{0} : {1} : {2} {3}",
                                    item.Name,
                                    item is PlayerMobile ? ((Account)item.Account).Username : "Null",
                                    item.Map,
                                    item.Location
                                ))
                        );
        ...
        public virtual void OnResponse( NetState state, RelayInfo info )
        {
                ...
                default:
                    {
                        ...
                        if ( index >= 0 && index < m_Names.Count )
                        {
                            ...
                            else if ( bringing )
                            {
                                if (sought != null)
                                {
                                    from.SendMessage($"Bringing {sought.Name} to you.");
                                    sought.Map = from.Map;
                            ...
                            else
                            {
                                if (sought != null)
                                {
                                    from.SendMessage($"Going to {sought.Name}.");
